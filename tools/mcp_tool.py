...
class MCPServerTask:
            ...
            async with httpx.AsyncClient(**client_kwargs) as http_client:
                async with streamable_http_client(url, http_client=http_client) as (
                    ...
                    async with ClientSession(read_stream, write_stream, **sampling_kwargs) as session:
                        ...
                        await self._discover_tools()
                        self._ready.set()
                        # Start health ping monitor as background task
                        health_task = asyncio.create_task(
                            self._health_ping_monitor(session)
                        )
                        try:
    ...
    async def _health_ping_monitor(self, session):
        """Background task: ping the MCP server periodically.
    
        Sets _reconnect_event when consecutive pings fail, which causes
        _wait_for_lifecycle_event() to return "reconnect" and tear down
        the session for a fresh connection.
        """
            ...
            while not self._shutdown_event.is_set():
                ...
                try:
                    ...
                    if consecutive_failures >= _MAX_FAILURES:
                        logger.warning(
                            "MCP server '%s' session unhealthy after %d "
